...
/**
 * [CBD-19] 301 redirects for old-site URLs that still rank.
 *
 * A filterable PHP map rather than .htaccess rules, for three
 * reasons: it's version-controlled with the instance (htaccess gets
 * regenerated by hosts/plugins and edits silently vanish), matching
 * runs on the DECODED path so old URLs carrying encoded junk
 * (%C2%A2 etc.) match reliably, and clones inherit the mechanism
 * with an empty map. Only mapped paths redirect — everything else
 * falls through to the smart 404 ([CBD-18]), and the homepage is
 * never mapped (it ranks; it stays).
 *
 * Map shape: normalized old path => absolute or site-relative
 * destination. Normalization (both sides): rawurldecode, lowercase,
 * strip query/fragment, collapse to /leading/trailing/ slashes.
 */
function affiliate_master_normalize_redirect_path( $path ) {
	$path = strtolower( rawurldecode( (string) $path ) );
	$path = (string) wp_parse_url( $path, PHP_URL_PATH );
	return '/' . trim( $path, '/' ) . '/';
}

/**
 * Issue the 301 when the requested path is in the map.
 */
function affiliate_master_legacy_redirects() {
	if ( is_admin() ) {
		return;
	}

	$map = apply_filters( 'affiliate_master_redirect_map', array() );
	if ( ! $map ) {
		return;
	}

	$request = isset( $_SERVER['REQUEST_URI'] ) ? wp_unslash( $_SERVER['REQUEST_URI'] ) : ''; // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Normalized and compared against a fixed allowlist; never echoed.
	$path    = affiliate_master_normalize_redirect_path( $request );

	// The homepage normalizes to '//' — never redirected, it ranks.
	if ( '//' === $path ) {
		return;
	}
	...
	foreach ( $map as $old => $new ) {
		$normalized_map[ affiliate_master_normalize_redirect_path( $old ) ] = $new;
	}

	if ( isset( $normalized_map[ $path ] ) ) {
		wp_safe_redirect( esc_url_raw( home_url( $normalized_map[ $path ] ) ), 301 );
		exit;
	}
}
add_action( 'template_redirect', 'affiliate_master_legacy_redirects', 1 );

/**
 * The CBD instance's redirect entries (old ranking URLs -> current
 * category archives). Populated from Nathan's redirect map; unmapped
 * old URLs intentionally fall through to the smart 404.
 *
 * @param array $map Existing entries.
 * @return array
 */
function cbd_redirect_map( $map ) {
	return array_merge(
		$map,
		array(
			// From CBD-301-redirect-map.csv (2026-07-14): 33 old
			// ranking URLs -> current category archives. The 6
			// no-match rows are deliberately absent (smart 404 takes
			// them) and the homepage is never mapped.
			'/product/savage-cbd-full-spectrum-tincture-oil-orange-1000mg/' => '/product-type/oil-tincture/',
			'/product/reliva-cbd-wellness-relief-cream-stick-250mg-30ml-bottle/' => '/product-type/topical/',
			'/product/barleans-ideal-cbd-hemp-oil-for-dogs-2/' => '/product-type/pet/',
			'/product/nuleaf-naturals-full-spectrum-cbd-oil-300mg-5ml/' => '/product-type/oil-tincture/',
			'/product/pluscbd-oiln%C2%A2-cbd-softgels-gold-formula-15mg-4/' => '/product-type/capsule-tablet/',
			'/product/elixinol-cbd-hemp-oil-drops-daily-balance-natural/' => '/product-type/oil-tincture/',
			'/product/lazarus-naturals-cbd-capsules-50mg-no-longer-available-2/' => '/product-type/capsule-tablet/',
			'/product/pluscbd-oiln%C2%A2-cbd-softgels-gold-formula-15mg-2/' => '/product-type/capsule-tablet/',
			'/product/nuleaf-naturals-full-spectrum-pet-cbd-oil/' => '/product-type/pet/',
			'/product/penguin-cbd-tincture-oil-mint-2/' => '/product-type/oil-tincture/',
			'/product-category/spero-cbd-pet-treats-bacon-sweet-potato-2mg-are-thc-free-and-offer-soothing-and-calming-effects-for-your-beloved-pets/' => '/product-type/pet/',
			'/product/yum-yum-gummies-cbd-full-spectrum-gummy-bears-1500mg/' => '/product-type/gummies/',
			'/product/social-cbd-oil-tincture-drops-cinnamon-leaf-2/' => '/product-type/oil-tincture/',
			'/product/swiss-relief-cbd-gummies-turmeric/' => '/product-type/gummies/',
			'/product/veritas-farms-cbd-salve-unscented-2/' => '/product-type/topical/',
			'/product/purekana-hemp-dog-treats-ai-steak-sweet-potato/' => '/product-type/pet/',
			'/product/physicians-grade-cbd-roll-on-relief-cool-mint-750mg/' => '/product-type/topical/',
			'/product/savage-cbd-vape-juice-driven-3/' => '/product-type/vape/',
			'/product/cbdfx-cbd-terpene-vape-juice-og-kush/' => '/product-type/vape/',
			'/product/hydrocanna-ultra-cbd-balm-400mg/' => '/product-type/topical/',
			'/product/nanocraft-cbdn%C2%A2-cbd-superfood-green-powder-600mg/' => '/product-type/edible/',
			'/product/medipets-cbd-dog-treats-burger-beef-minis-300mg/' => '/product-type/pet/',
			'/product/hempceutix-relief-cbd-capsules-300mg-60-count/' => '/product-type/capsule-tablet/',
			'/product/hempceutix-stress-cbd-capsules-300mg-60-count/' => '/product-type/capsule-tablet/',
			'/product/chill-plus-delta-8-force-squares-gummies-island-mix-1000x/' => '/product-type/gummies/',
			'/product/koi-cbd-vape-juice-watermelon-green-apple-sour/' => '/product-type/vape/',
			'/product/koi-cbd-healing-balm-2/' => '/product-type/topical/',
			'/product/solari-hemp-cbd-roll-on-gel-warming-muscle-recovery-200mg/' => '/product-type/topical/',
			'/product/miss-buds-cbd-under-eye-oil-15mg/' => '/product-type/oil-tincture/',
			'/product/seven-points-cbd-tincture-600mg/' => '/product-type/oil-tincture/',
			'/product/joy-organics-cbd-tincture-oil-tranquil-mint/' => '/product-type/oil-tincture/',
			'/product/chill-plus-cbd-infused-gummy-bears-bundle/' => '/product-type/gummies/',
			'/product/naked-100-hawaiian-pog-cbd-vape-juice-30ml-2/' => '/product-type/vape/',
		)
	);
}
add_filter( 'affiliate_master_redirect_map', 'cbd_redirect_map' );
