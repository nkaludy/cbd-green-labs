...
namespace PrettyLinks\Caseproof\GrowthTools\Helper;
...
class AddonHelper
{
    ...
    public static function activateAddon(string $addon, string $addonType = 'plugin')
    {
        ...
        if (check_ajax_referer('caseproof_growth_tools_install_addon', 'nonce')) {
            ...
            if (! current_user_can('activate_plugin')) {
                wp_send_json_error(
                    esc_html__('Could not activate addon. Please check user permissions.', 'pretty-link')
                );
            }
            ...
            if (isset($addon)) {
                ...
                if (! is_wp_error($activate)) {
                    wp_send_json_success(esc_html__('Addon activated.', 'pretty-link'));
                }
            }

            wp_send_json_error(
                esc_html__('Could not activate addon. Please activate from the Plugins page.', 'pretty-link')
            );
        }

        wp_send_json_error(
            esc_html__('Could not activate addon. Please refresh page and try again.', 'pretty-link')
        );
    }
    ...
    public static function deactivateAddon(string $addon, string $addonType = 'plugin')
    {
        ...
        if (! current_user_can('activate_plugins')) {
            wp_send_json_error(
                esc_html__('Could not deactivate addon. Please check user permissions.', 'pretty-link')
            );
        }
    ...
    public static function installAddon(string $addon, string $addonType = 'plugin')
    {
        ...
        if (! current_user_can('install_plugins')) {
            wp_send_json_error(
                esc_html__('Could not install addon. Please check user permissions.', 'pretty-link')
            );
        }
